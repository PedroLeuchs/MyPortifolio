...
const HomePage: React.FC<HomePageProps> = ({ section, language, indexImg }) => {
  ...
  return (
    <section
      ...
      key={section.url}
      className="font-poppins h-screen flex items-center justify-center text-white text-3xl font-bold transition-all duration-700 ease-in-out transform bg-neutral-800"
    >
      <img
        src="/assets/datail_extra.svg"
        alt="Detalhe"
        className={`h-screen absolute left-44 animate-pulse drop-shadow-lg-custom ${
          indexImg % 2 === 0 ? "rotate-180 " : ""
        }`}
      />
      <div className="w-full h-1/3 flex items-center justify-center backdrop-blur-[15px] bg-neutral-700/40 shadow-2xl shadow-neutral-800/50 py-20">
        <div className="w-1/3 h-full flex items-start justify-center flex-col">
          <div className="text-6xl font-bold text-white flex text-start items-start justify-start">
            {displayTextTitle}
            <span className="text-6xl text-indigo-600 flex animate-bounce ml-2">
              !
            </span>
          </div>
